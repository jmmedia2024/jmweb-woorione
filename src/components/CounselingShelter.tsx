  ...
  return (
    <div className="space-y-12 animate-fade-in text-slate-800" style={{ fontSize: `${16 * fontRatio}px` }}>
      ...
      <div className="border-b border-slate-100 pb-5 max-w-2xl">
        <span className="text-xs font-black uppercase text-blue-600 tracking-wider">Hot-Line & Shelter Portal</span>
        <h1 className="text-3xl font-black text-slate-900 mt-1">통일마중쉼터 입소 및 상담 핫라인</h1>
        <p className="text-slate-500 font-semibold mt-1">외로운 정착 길에 따스한 등불을 밝혀드리는 1:1 전문 케어 서비스입니다.</p>
      </div>

      {/* 3. Direct Simulated Hotline Dialing widget Component */}
      <section className={`p-8 rounded-3xl border text-center relative overflow-hidden transition-all duration-300 ${
        highContrast 
        ...
        <div className="max-w-xl mx-auto space-y-6 relative z-10">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-black">우리원 24시간 정서안정 안심 핫라인</h2>
          ...
          <div className="p-4 bg-white/10 rounded-2xl backdrop-blur-md max-w-sm mx-auto border border-white/20">
            <p className="text-lg font-black tracking-widest text-orange-200">📞 직통전화 : 041-543-9060</p>
          ...
          <button
            ...
            className="px-8 py-4 bg-orange-500 hover:bg-orange-600 text-white rounded-2xl text-lg font-extrabold shadow-md inline-flex items-center gap-2 animate-pulse transition-all cursor-pointer"
          >
        ...
        {isCalling && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div className={`p-8 rounded-3xl max-w-md w-full border text-left space-y-6 relative ${
              highContrast ? "bg-black border-yellow-400 text-yellow-300" : "bg-white text-slate-800"
            }`}>
              <button 
                id="close-hotline-sim-btn"
                onClick={endHotlineCall}
                className="absolute top-4 right-4 p-2 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-900"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-4">
                <p className="text-base text-slate-700 leading-relaxed font-bold bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <button
                id="end-call-btn"
                onClick={endHotlineCall}
                className="w-full py-3.5 bg-red-650 hover:bg-red-700 text-white font-extrabold rounded-xl text-base shadow transition-all cursor-pointer"
              >
                상담 종료 및 대기
              </button>
            </div>
        ...
        <div className="lg:col-span-7 bg-white p-6 md:p-8 rounded-3xl border border-slate-150 shadow-sm space-y-6">
          <div className="space-y-1">
            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-1.5">
              <UserPlus2 className="w-5.5 h-5.5 text-blue-600" />
              <span>간편 비공개 심리 코칭 & 쉼터 예약</span>
            </h3>
            <p className="text-slate-500 text-xs">수혜자의 안전과 신원 한계 보호를 위해 이름은 마스킹 처리되어 보존됩니다.</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-extrabold text-slate-700">이름 / 가명 (Alias)</label>
                <input
                  ...
                  className="w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 text-base"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-extrabold text-slate-700">실시간 연락처 (Phone)</label>
                <input
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-extrabold text-slate-700">나이 / 연세</label>
                <input
                  ...
                  placeholder="예: 62"
                  className="w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 text-base animate-none"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-extrabold text-slate-700">요구 지원 항목 (Type)</label>
                <select
                  id="counsel-type-select"
                  value={type}
                  onChange={e => setType(e.target.value as any)}
                  className="w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 text-base font-bold text-slate-755 bg-white"
                >
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-extrabold text-slate-700">상담 및 위기 정황 요약 (Message)</label>
              <textarea
                ...
                className="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 text-base"
              ></textarea>
            </div>
            ...
            <button
              ...
              type="submit"
              className="w-full py-4 bg-slate-900 text-white font-extrabold rounded-xl text-lg hover:bg-slate-800 transition-all cursor-pointer"
            >
              상담 예약 및 위기 구제 원서 접수
            </button>
          </form>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-150 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h4 className="font-bold text-slate-900">실시간 상담 매칭 현황판</h4>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {requests.map((r) => (
                <div key={r.id} className="p-4 bg-white rounded-xl border border-slate-100 shadow-3xs space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-400 font-bold">{r.createdAt}</span>
                    <span className={`text-xxs px-2 py-0.5 rounded-full font-black ${
                      r.status === "코치 배정완료"
                  ...
                  <p className="text-slate-500 text-xs truncate italic">{r.message}</p>
                </div>
              ))}
            </div>

            <div className="pt-2 bg-blue-50/50 p-3.5 rounded-xl border border-blue-100/50 space-y-1 text-xxs text-slate-500 font-semibold leading-relaxed">
              <p className="font-extrabold text-slate-700 text-xs">🔒 개인 프라이버시 특별 규범</p>
